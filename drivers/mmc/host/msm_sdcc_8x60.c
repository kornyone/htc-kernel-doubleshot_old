...
#endif
	/* This is a workaround for SD power issue */
	/* Disable SD power here */
	if (is_sd_platform(plat)) {
		if (host->plat->translate_vdd) {
			host->plat->translate_vdd(&(pdev)->dev, 1);
			mdelay(30);
			host->plat->translate_vdd(&(pdev)->dev, 0);
		}
	}
